...
class SitemapController extends Controller
{
    ...
    public function store(){
        ...
        return view('frontend.sitemap.update',compact('engines_sitemap','automobiles_sitemap'));
    }

    public function data_automobiles()
    {
        $autpmobiles = Automobiles::all('updated_at', 'url')->toArray();
        return $this->sitemap_template($autpmobiles, 3000, 'url', 'updated_at', 'models_sitemap');
    }
...
//$name_file - The name of the final file after processing
    public function sitemap_template($input_data, $paginate, String $url, String $date, String $name_file)
    {

        $page_number = $paginate;
        $page_start = 0;
        $step = (count($input_data) / $page_number) + 1;
        for ($sitemap = 1; $sitemap <= $step; $sitemap++) {
            ...
            for ($i = $page_start; $i < $page_number; $i++) {
            ...
            $page_number = $page_number + $page_start;
        }
    }
